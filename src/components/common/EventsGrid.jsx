...
class EventsGrid extends React.Component {
    ...
    render() {
        ...
        const eventArgs = this.props.events.map((event) =>
            shortenEvent(event, rowKey)
        );
                ...
                ? {
                      filterDropdown: (
                          <Radio.Group
                              ...
                              onChange={(e) =>
                                  this.handleModeChange(column, e.target.value)
                              }
                          >
        ...
        const columns = this.props.event.inputs.map((input, index) => ({
            ...
            render: (value) => (
                <FormattedValue
                    type={input.type}
                    value={value}
                    mode={this.getUintMode(input.name)}
                />
            ),
        }));
        ...
        return (
            <Table
                dataSource={eventArgs}
                rowKey={rowKey}
                columns={columns.concat(systemColumns)}
                size="small"
                scroll={{ x: true }}
            ></Table>
        );
